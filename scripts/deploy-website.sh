...
# Screenshots referenced by docs.html / download.html (were never deployed).
ssh -i "$SSH_KEY" "$SSH_TARGET" 'mkdir -p /opt/bitnami/nginx/html/screenshots'
scp -i "$SSH_KEY" -r website/screenshots/. "$SSH_TARGET:/opt/bitnami/nginx/html/screenshots/"
