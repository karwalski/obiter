...
**Status:** FIXED (cases.ts — `formatParallelCitations`/`formatReportedCase` JSDoc and data contract now state the Rule 2.2.7 prohibition; parallels documented as court-mode/foreign-style only) + FIXED (engine wiring wave 2 — dispatchReportedCase passes parallelCitations only when writingMode === "court"; dispatch test in tests/engine/parity-dispatch.test.ts). The validator.ts inversion was fixed by the validator agent (wave 1).
...
**Status:** FIXED (cases-unreported.ts — `judges` + `pinpoint` (post-parenthesis, no comma) added; `proceedingNumber` deprecated and never emitted. Tests assert exs 84 and 95 verbatim) + FIXED (engine wiring wave 2 — dispatchUnreportedNoMnc passes judges + pinpoint, drops proceedingNumber; exact ex 84 dispatch test).
...
**Status:** FIXED (cases-unreported.ts — roman 'Order of «officers» in ' head, optional `proceedingNumber`. Tests assert exs 88 and 89 verbatim) + FIXED (engine wiring wave 2 — dispatchCourtOrder passes judicialOfficers + proceedingNumber; exact ex 88 dispatch test).
...
**Status:** FIXED (engine wiring wave 2 — engine.ts appendFirstCitationSuffixes now suppresses the introduction only when the short title equals the whole rendered citation; exact ex 40 dispatch test in tests/engine/parity-dispatch.test.ts).
...
**Severity:** Medium.
**Status:** FIXED (cases.ts — `COURT_DISPLAY` maps identifiers to court names with `unqualifiedName` jurisdiction-suppressed forms; `SERIES_JURISDICTION` drives suppression (ex 77: '(Court of Appeal)' with Qd R); unknown ids pass through as names; `formatReportedCase` gains `judicialOfficers` runs emitted before the court parenthetical, asserted against ex 79) + FIXED (engine wiring wave 2 — dispatchReportedCase formats officers first and passes them into formatReportedCase; post-hoc append deleted; exact ex 79 dispatch test).

### 2.3.1 Decisions with MNC (PDF pp.79–81) — GAP, Medium (+ anomaly check: clean)

**Status (data half):** FIXED (court-identifiers.ts: HCASL/FamCA/FamCAFC/NTCCA/TASCCA added with rule 2.3.1 years; `mncFrom`/`mncTo` fields added for year-threshold validation; reportHierarchy.ts: TASSC/TASCCA/NTCCA/HCASL resolve) / FIXED (wave 2: validator.ts `checkMncYearValidity` flags MNC years predating `mncFrom`, wired into validateDocument, tests in validator-new.test.ts; TASCSC→TASSC preset-key rename completed across presets.ts, practiceDirections.ts, validator.ts, courtReferenceGuide.ts and the legacy reportHierarchy.ts alias deleted).
...
**Severity:** Medium.
**Status:** FIXED (cases-supplementary.ts — `JudicialOfficerRef` gains `role: "for_the_court"` and `onBehalfOf: Array<{name; title}>` (grouped/pluralised per 2.4.5). Tests assert exs 100 and 101 verbatim; empty-title officers no longer emit a stray space, asserted via ex 79) + FIXED (engine wiring wave 2 — dispatchReportedCase types officers as JudicialOfficerRef[], passing role/agreeingAt/judgmentGroup/onBehalfOf through. UI form fields wave 3).
...
- **2.2.4 (PDF p.77):** `startingPage` is `number` throughout (`cases.ts:360`), so unique-reference citations ('(2002) EOC ¶93-198', ATPR '¶41-703, 43,014') cannot be represented. GAP Low.
  **Status:** FIXED (cases.ts — `startingPage: number | string` in `formatStartingPageAndPinpoint`/`ReportedCaseData`; exs 67 and 75 asserted verbatim) + FIXED (engine wiring wave 2 — dispatchReportedCase passes non-numeric startingPage strings through; exact ex 67 dispatch test).
...
- **2.6.2 (PDF p.89):** `formatArbitration` (`cases-supplementary.ts:240-251`) emits `parties (type) awardDetails` — template wants one parenthetical `(«Award Description», «Forum», «Case/Award No», «Date»)` then pinpoint, and a 'reported in' tail for reproduced awards; neither is modelled (achievable only by field-stuffing). Low.
  **Status:** FIXED (cases-supplementary.ts — full template fields incl no-parties variant (no parentheses, comma before pinpoint) and 'reported in' tail; legacy fields preserved for stored data. Exs 112, 113, 115 asserted verbatim) + FIXED (engine wiring wave 2 — dispatchArbitration passes the full 2.6.2 template fields (awardDescription/forum/caseNumber/date/pinpoint/reportedIn) with legacy fallback; dispatchQuasiJudicial passes pinpoint/separator. UI fields wave 3).
...
- **TASCC (2.3.1 worked example typo):** clean — no `TASCC` in src or tests; `TASSC` used correctly (`court-identifiers.ts:143`, `cases.ts:34`, `reportHierarchy.ts:124`). But note the engine's own novel typo `TASCSC` at `reportHierarchy.ts:123` and the *missing* `TASCCA`.
  - **Status:** FIXED — TASCCA added to court-identifiers.ts and reportHierarchy.ts; wave 2 completed the coordinated TASCSC→TASSC preset-key rename (presets.ts, practiceDirections.ts, validator.ts, courtReferenceGuide.ts) and deleted the legacy `TASCSC: "TAS"` alias from reportHierarchy.ts — "TASCSC" no longer exists in src (only as negative assertions in tests).
