...
**Status (X-1):** FIXED (engine wiring landed: engine.ts now imports all twelve `rules/v4/foreign/*` modules and `dispatchForeign` routes cases (`dispatchForeignCase`/`dispatchForeignCourtDecision`), legislation (`dispatchForeignLegislation` — constitutions, regulations, SIs, regnal years, Fed Reg) and secondary sources (`dispatchForeignSecondary`) to the per-jurisdiction formatters, wave 2 + wave-3 mop-up. The underlying formatter mismatches (16.x, 17.1, 18.1, 24.1.3, 24.1.4, 25.3, 25.7, 26.1.1, 26.2) were FIXED in wave 2; docs/aglc4-coverage.md Part V refreshed in the final wave (PARITY-119).)
...
**Status (X-2):** FIXED (docs/aglc4-audit.md rebased row-by-row and docs/aglc4-coverage.md no longer claims "Full" for the 12 chapters — PARITY-120; Part V rows refreshed again after the wave-3 dispatch mop-up in the final wave).
