...
- **Status:** FIXED (engine wiring wave 2 — engine.ts `appendFirstCitationSuffixes` suppresses the introduction only on strict equality of short title and full rendered citation; exact ex 29/40 tests in tests/engine/parity-dispatch.test.ts).
...
- **Status:** FIXED (src/engine/resolver.ts — `formatShortReference`, `formatShortTitleIntroduction`, and the court-mode/above-n variants emit `legislation.bill` short titles roman (Rules 3.2/3.5); Acts/delegated legislation unchanged (italic). Tests in tests/engine/chapter1.test.ts).
...
- **Status:** FIXED (engine wiring wave 2 — dispatchStatute/dispatchBill consume `data.legislativeHistory` via formatLegislativeHistory; exact fn 61 dispatch test in tests/engine/parity-dispatch.test.ts. UI opt-in section is wave 3 — see engine-leftovers.md).
...
- **Status:** FIXED (engine wiring wave 2 — dispatchConstitution collapses to formatCommonwealthConstitution only for sanctioned aliases/empty titles; exact ex 49/50 dispatch tests).
...
- **Status:** FIXED (src/engine/rules/v4/domestic/legislation-supplementary.ts — `formatGazette` gains `noticeAuthor`/`noticeTitle` (roman, quoted, `in`), `page: number | string` starting page and `pinpoint` string; exact tests for exs 69–71). Dispatch pass-through: FIXED (engine wiring wave 2 — dispatchQuasiLegislative gazette branch passes noticeAuthor/noticeTitle/pinpoint and string pages).
