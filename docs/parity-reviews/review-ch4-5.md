...
### 4.2 Title — GAP (Medium)
**Status:** FIXED (src/engine/rules/v4/secondary/general.ts — dash→colon normalisation, first-subtitle truncation with date-span exception, rule-1.7 casing via the repaired toTitleCase) · embedded italics FIXED (PARITY-122 / DECISION-021: asterisk marker convention parsed at format time in src/engine/rules/v4/general/titleMarkup.ts — no store schema change; ex 2 (*IceTV*) and ex 26 (*Briginshaw*) asserted exactly; inside wholly italic titles marked spans stay italic per rule 4.2 'no part in roman', PDF p.113; UI affordance/help copy HANDOFF (handoff/title-markup.md))
