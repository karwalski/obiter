...
**Status:** FIXED (src/engine/rules/v4/secondary/books.ts — bare 'rev ed' for unnumbered revisions (ex 17), `revised` flag on book data) · dispatch passthrough FIXED (engine wiring wave 2 — src/engine/engine.ts; engine-dispatch tests in tests/engine/parity-dispatch.test.ts; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/books.ts — `year: number | string` (spans '1984–88', '1975–'); 0/empty year omitted, fixing '(Publisher, 0)') · dispatch passthrough FIXED (engine wiring wave 2 — src/engine/engine.ts; engine-dispatch tests in tests/engine/parity-dispatch.test.ts; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/books.ts — `volumeLabel: "vol" | "bk"` (ex 27), string volumes, span years; exact-match tests exx 26, 27) · dispatch routing FIXED (engine wiring wave 2 — src/engine/engine.ts; engine-dispatch tests in tests/engine/parity-dispatch.test.ts; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/books.ts — `editors` field emits ', ed «names»' after the title, never 'eds'; exact-match tests exx 34, 35) · dispatch FIXED (engine wiring wave 2 — src/engine/engine.ts; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/books.ts — formatter verified, exact-match test ex 40) · reachability FIXED (wave 2 — dispatchBook routes formatForthcomingBook on d.forthcoming; dispatchBookEbook JSDoc corrected to non-AGLC extension; decisions.md entry wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — committee now precedes the legislature; `legislature` field + pinpoint added; exact-match tests exx 8, 12) · dispatch passthrough FIXED (engine wiring wave 2 — src/engine/engine.ts dispatchParliamentaryReport)
...
### 7.1.3 Royal commission reports — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — authorless, '(Document Type, Full Date)' parenthetical, string dates, vol + pinpoint; exact-match tests exx 13, 15) · dispatch FIXED (engine wiring wave 2 — dispatchRoyalCommission authorless, documentType/date/pinpoint; UI relabel wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — number optional (no 'No ' for unnumbered papers), full-date support, pinpoint + URL; exact-match tests exx 28, 30) · dispatch passthrough FIXED (engine wiring wave 2 — src/engine/engine.ts; UI date fields wave 3)
...
### 7.2.3 Parliamentary research papers — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — individual author leads; provider + legislature + full date inside the parenthetical; exact-match tests exx 33, 36) · dispatch passthrough FIXED (engine wiring wave 2 — src/engine/engine.ts; UI date fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — `speechType` replaces 'Speech' with a leading 'The' stripped; exact-match tests exx 42, 43) · dispatch FIXED (engine wiring wave 2 — dispatchSpeech speechType/lectureName; UI field wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — `releaseType`, `documentNumber` (ex 45), in-parenthetical `issuingBody` dropped when equal to the author, pinpoint; exact-match tests exx 45, 46) · dispatch passthrough FIXED (engine wiring wave 2 — dispatchPressRelease; issuingBody removed from author fallback)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other.ts — inquiry italic and omitted when unstated (stray-comma bug gone, ex 54), pinpoint added; exact-match tests exx 49, 54) · dispatch pinpoint passthrough FIXED (engine wiring wave 2 — dispatchSubmissionGovernment)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — `location` field added; `jurisdiction` kept as deprecated alias; exact-match tests exx 55, 56) · dispatch FIXED (engine wiring wave 2 — dispatchParliamentaryEvidence location); UI relabel wave 3
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — title italic, `(speaker)` suffix, non-AGLC vol slot no longer emitted; exact-match tests exx 57, 58) · dispatch passthrough FIXED (engine wiring wave 2 — dispatchConstitutionalConvention speaker added, volume dropped)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — publisher no longer emitted, online '(online at …)' form, homograph `entryType` marker; exact-match tests exx 59, 61, 62) · dispatch FIXED (engine wiring wave 2 — dispatchDictionary retrievedDate/entryType; publisher no longer passed); UI wave 3
...
### 7.7 Legal encyclopedias — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — publisher leads, vol before '(at …)', title number/name, online form, bracketed paragraphs; exact-match tests exx 64, 65) · dispatch passthrough FIXED (engine wiring wave 2 — dispatchLegalEncyclopedia/dispatchLooseleaf)
...
### 7.8 Looseleaf services — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — 'Author, Publisher, *Title*, vol N (at Service/Date) [para]' order, online form, ¶ passthrough; exact-match tests exx 67, 70) · dispatch passthrough FIXED (engine wiring wave 2 — dispatchLegalEncyclopedia/dispatchLooseleaf)
...
### 7.9 IP materials — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — italic identifier segment, 'filed/lodged on', status parenthetical; invented Title/Applicant deprecated; exact-match tests exx 71, 76) · dispatch FIXED (engine wiring wave 2 — dispatchIpMaterial new fields; title/applicant no longer passed); UI rebuild wave 3
...
### 7.10 Constitutive documents — MISMATCH (High)
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — '*DocType*, Company (at Date) pinpoint', company designators stripped; exact-match test ex 78) · dispatch date passthrough FIXED (engine wiring wave 2 — dispatchConstitutiveDocument)
...
**Status:** FIXED (engine wiring wave 2 — dispatchPeriodical: date parenthetical before the italic periodical name per ex 89, vol/issue branch removed with a JSDoc pointer to journal.article; test in tests/engine/parity-dispatch.test.ts)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — 'Editorial, ' always leads with the quoted title kept (ex 91); `titleIsDescription` unquoted descriptions (ex 92); masking substring tests replaced) · dispatch route FIXED (engine wiring wave 2 — dispatchNewspaper routes isEditorial → formatEditorial, passes titleIsDescription; UI checkboxes wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — 'Directed by' removed; production company + version details + time pinpoint; exact-match tests exx 99, 100) · dispatch rework FIXED (engine wiring wave 2 — dispatchFilmTvMedia film/TV branches reworked; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — 'Season X, Episode Y' built in the episode-title slot, empty parts filtered (dangling ', )' gone), time pinpoint + URL; exact-match tests exx 101, 103, 105) · dispatch rework FIXED (engine wiring wave 2 — dispatchFilmTvMedia film/TV branches reworked; UI fields wave 3)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — producer element (omitted when equal to series title), time pinpoint; exact-match tests exx 106, 108, 109) · dispatch route FIXED (engine wiring wave 2 — dispatchFilmTvMedia routes medium Podcast/Radio → formatPodcast)
...
**Status:** FIXED (src/engine/rules/v4/secondary/other-media.ts — `documentType` field ('(Type, Date)' / '(Type)'), author omitted when identical to the website; smuggled-date tests rewritten to exx 112, 113) · dispatch FIXED (engine wiring wave 2 — dispatchInternetMaterial documentType with legacy-date guard); UI select wave 3
