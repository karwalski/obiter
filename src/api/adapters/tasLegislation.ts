 ...
 *
 * TAS Legislation Adapter (Story 17.30)
 *
 * Scrapes legislation.tas.gov.au for Tasmanian Acts and regulations.
 */
...
export class TasLegislationAdapter extends StateLegislationAdapter {
  readonly descriptor: SourceAdapterDescriptor = {
    id: "tas-legislation",
    displayName: "TAS Legislation",
    jurisdictions: ["AU-TAS"],
    contentTypes: ["legislation"],
    accessTier: "live",
    licence: "Tasmanian Government",
    requiresKey: false,
    rateLimitHint: { requestsPerSecond: 1, burst: 2 },
    fragile: true,
  };

  protected readonly baseUrl = "https://legislation.tas.gov.au";
  ...
  parseSearchResults(html: string): LookupResult[] {
    ...
    while ((match = pattern.exec(html)) !== null) {
      ...
      results.push({
        title,
        snippet: `TAS legislation: ${title}`,
        sourceId: id,
        confidence: 0.7,
        sourceUrl: `${this.baseUrl}/view/html/inforce/${id}`,
        attribution: "Tasmanian Government",
      });
    }
    return results;
  }
