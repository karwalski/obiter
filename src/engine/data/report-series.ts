...
export interface ReportSeriesEntry {
  ...
  /**
   * True where `type` is "medium_neutral". Redundant with `type` since the
   * union migration; retained because ranking logic in
   * src/engine/court/reportHierarchy.ts still reads it (engine-owned —
   * switch that read to `type === "medium_neutral"` before deleting this).
   */
